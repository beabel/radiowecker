...
boolean initWiFi(const char *wifiSsid, const char *wifiPkey) {
  ...
  RADIO_SERIAL(Serial.print(TXT_DISCONNECT));          // Debug-Ausgabe, dass die Verbindung getrennt wird
  WiFi.disconnect();                     // Trennt die aktuelle WiFi-Verbindung
  RADIO_SERIAL(Serial.print(TXT_SOFT_AP_DISCONNECT));  // Debug-Ausgabe, dass der Soft-AP-Modus getrennt wird
  ...
  RADIO_SERIAL(Serial.print(TXT_SET_WLAN_MODE));  // Debug-Ausgabe, dass der WLAN-Modus gesetzt wird
  ...
  if (wifiSsid && wifiSsid[0] != '\0') {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      RADIO_SERIAL(Serial.print(TXT_IP_ADDRESS));    // Debug-Ausgabe der IP-Adresse
  ...
  if (!connected) {
    RADIO_SERIAL(Serial.println(TXT_NO_CONNECTION));  // Debug-Ausgabe, dass keine Verbindung hergestellt werden konnte
