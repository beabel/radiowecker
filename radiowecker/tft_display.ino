...
TS_Point transformTouch(TS_Point p) {
  ...
  switch (tft.getRotation()) {
    ...
    // Bei Rotation 1 
      ...
      break;
    case 3:
      // Keine Änderung notwendig, da pTransformed bereits p ist
      break;
    default:
      // Standardverhalten, keine Änderung notwendig
      break;
  }
...
void onTouchClick(TS_Point p) {
  // Zuerst die Touch-Koordinaten transformieren
  p = transformTouch(p);
...
void textInBox(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const char* text, uint8_t align = ALIGNLEFT, boolean big = false, uint16_t fc = ILI9341_WHITE, uint16_t bg = ILI9341_BLACK, uint8_t lines = 1) {
  ...
  if (big) tft.setFont(FNT12);
  else tft.setFont(FNT9);
  tft.setTextColor(fc);
